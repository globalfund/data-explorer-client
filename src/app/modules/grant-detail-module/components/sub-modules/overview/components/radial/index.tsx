...
export function InvestmentsRadialViz() {
  ...
  const grantInfoData = useStoreState((state) => ({
    disbursed: get(
      state.GrantDetailInfo.data,
      "data[0].investments.disbursed",
      0
    ),
    committed: get(
      state.GrantDetailInfo.data,
      "data[0].investments.committed",
      0
    ),
    signed: get(state.GrantDetailInfo.data, "data[0].investments.signed", 0),
  }));

  const total =
    grantInfoData.committed + grantInfoData.signed + grantInfoData.disbursed;
  const values = [
    grantInfoData.committed,
    grantInfoData.signed,
    grantInfoData.disbursed,
  ];
  ...
  }, [grantInfoData.disbursed, grantInfoData.committed, grantInfoData.signed]);
      ...
      radialBar: {
        ...
        dataLabels: {
          ...
          total: {
            show: true,
            label: `${Math.floor(
              (grantInfoData.disbursed / grantInfoData.committed) * 100
            )}%`,
    ...
    >
      ...
      >
        <div>
          ...
          <div>{formatLargeAmountsWithPrefix(grantInfoData.committed)}</div>
        </div>
        <div>
          <span>
          ...
          <div>{formatLargeAmountsWithPrefix(grantInfoData.signed)}</div>
        </div>
        <div>
          <span>
          ...
          <div>{formatLargeAmountsWithPrefix(grantInfoData.disbursed)}</div>
        </div>
      </div>
    </div>
  );
